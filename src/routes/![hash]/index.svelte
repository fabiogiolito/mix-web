...
<div class="min-h-available lg:min-h-screen flex flex-col">
  ...
  {#if content}
    ...
    <div class="flex-1 flex flex-col-reverse lg:flex-row lg:space-x-10 px-4 lg:px-6">
      ...
      <div class="flex-1 flex relative lg:mb-10">
        <img src={content.image} class="lg:absolute top-0 left-0 w-full h-full object-contain" alt="content">
      </div>
...
<Modal bind:isOpen={likesModalOpen} showClose>
  ...
  <div class="space-y-6">
    {#each [...Array(20)] as _}
      <UserRow showFollow linked user={content?.sharedBy} />
    {/each}
  </div>
</Modal>
